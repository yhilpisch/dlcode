...
def main() -> None:
    ...
    for i in range(3):
        x = 0.1 + i * 0.28
        ax.add_patch(plt.Rectangle((x, 0.30), 0.22, 0.52, ec="#444", fc="#f7f7f7"))
        ...
        for j, (lab, fc) in enumerate(zip(labels, colors)):
            y0 = base_y + j * 0.15
            ax.add_patch(plt.Rectangle((x + 0.02, y0), 0.18, 0.12, ec="#666", fc=fc))
            ax.text(x + 0.11, y0 + 0.06, lab, ha="center", va="center", fontsize=8)

    # Lift the title further to prevent collisions with GPU labels
    ax.text(0.5, 0.94, "ZeRO: shard params/opt/grads across devices; all-gather on demand",
