...
def run(shuffle: bool, epochs=12):
    ...
    if shuffle:
        ...
        # Construct an adversarial, ordered loader (grouped by label) to
        # illustrate why shuffling helps.
        idx = torch.argsort(y_tr_t)
        X_tr_ord, y_tr_ord = X_tr_t[idx], y_tr_t[idx]
        loader = DataLoader(
            TensorDataset(X_tr_ord, y_tr_ord), batch_size=64, shuffle=False
        )
...
def main():
    out = Path("figures/ch09_shuffle_loss.svg")
    ...
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.title("Shuffle vs no-shuffle (loss)")
