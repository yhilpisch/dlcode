...
def main() -> None:
    out = Path('figures/ch13_toy_boundary.svg'); out.parent.mkdir(parents=True, exist_ok=True)
    ...
    Xc_pos = x_pos - mu_pos
    Xc_neg = x_neg - mu_neg
    S = (Xc_pos.T @ Xc_pos + Xc_neg.T @ Xc_neg) / (len(x_pos) + len(x_neg) - 2)
    ...
    ax.scatter(x_pos[:,0], x_pos[:,1], c='tab:green', label='positive', alpha=0.8)
    ax.scatter(x_neg[:,0], x_neg[:,1], c='tab:red', label='negative', alpha=0.8)
    ...
    ax.legend(frameon=False); ax.set_title('Toy decision boundaries')
    ax.set_xlabel('sentiment axis (toy)'); ax.set_ylabel('noise')
