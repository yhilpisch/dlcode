...
class SlowOnTheFly(Dataset):
    def __init__(self, n: int = 600, delay: float = 0.0004) -> None:
        self.n = n
        self.delay = delay
    ...
    def __getitem__(self, i: int):
        ...
        x = torch.tensor(
            [(i % 10) / 10.0, ((i * 7) % 10) / 10.0], dtype=torch.float32
        )
...
def main() -> None:
    out = Path("figures/ch09_cache_speedup.svg")
    ...
    plt.title("Caching heavy transforms speeds up I/O")
    plt.tight_layout()
