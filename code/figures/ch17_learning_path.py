...
def main() -> None:
    ...
    for label, x, y in base:
        ax.add_patch(
            plt.Rectangle((x, y), box_w, box_h, ec="#444", fc="#eef5ff")
        )
        ax.text(
            x + box_w / 2,
            y + box_h / 2,
    ...
    ax.add_patch(
        plt.Rectangle((cont_x, cont_y), cont_w, cont_h, ec="#444", fc="#ffffff")
    )
    ax.text(
        cont_x + cont_w / 2,
        cont_y + cont_h + 0.03,
    ...
    for i, label in enumerate(rows):
        ...
        ax.add_patch(
            plt.Rectangle((cont_x, y), cont_w, row_h, ec="#ddd", fc="#f8fbff")
        )
        ax.text(
            cont_x + 0.06,
            y + row_h / 2,
