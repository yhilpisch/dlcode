...
def main() -> None:
    ...
    for i, y in enumerate([3.2, 2.0, 0.8]):
        ...
        ax.text(0.25, y, f"x{i+1}", ha='right', va='center',
                bbox=dict(facecolor='white', edgecolor='none', alpha=0.8, pad=0.3))
        ax.text(1.8, (y+2.0)/2 + 0.25, f"w{i+1}", fontsize=10, color='#333',
    ...
    # Activation
    ax.annotate('', xy=(5.0, 2.0), xytext=(3.7, 2.0), arrowprops=dict(arrowstyle='->'))
    ax.text(4.1, 2.3, r'$\phi$', fontsize=12, bbox=dict(facecolor='white', edgecolor='none', alpha=0.7, pad=0.2))
    ax.annotate('', xy=(8.8, 2.0), xytext=(5.2, 2.0), arrowprops=dict(arrowstyle='->'))
